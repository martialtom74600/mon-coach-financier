...
export default function SettingsPageClient({
  ...
  const loadedFromServer = initialPrefs !== undefined;
  const [prefs, setPrefs] = useState<Preferences | null>(() =>
    loadedFromServer ? initialPrefs ?? null : null,
  );
  const [loading, setLoading] = useState(!loadedFromServer);
  ...
  useEffect(() => {
    ...
    fetchPrefs();
  }, [isLoaded, isSignedIn, showToast, router, loadedFromServer]);
  ...
  if (!prefs) {
    return (
      <div className="py-12 text-center">
        <p className="text-slate-600 mb-4">Complète ton profil pour accéder aux réglages.</p>
  ...
  return (
    <div className="w-full animate-fade-in pb-20 md:pb-0">
      <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
        ...
        <GlassCard>
          <h2 className="text-lg font-bold text-slate-800 mb-4">Données & vie privée</h2>
          <RgpdSection
